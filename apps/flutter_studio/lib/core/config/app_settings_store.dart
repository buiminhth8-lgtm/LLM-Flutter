...
class AppSettingsStore {
  Future<AppSettings> load() async {
    ...
    return AppSettings(
      apiBaseUrl:
          prefs.getString('llm_studio.api_base_url') ?? defaults.apiBaseUrl,
      ...
      chatStreamingEnabled:
          prefs.getBool('llm_studio.chat_streaming_enabled') ??
          defaults.chatStreamingEnabled,
      autoStartBackend:
          prefs.getBool('llm_studio.auto_start_backend') ??
          defaults.autoStartBackend,
      closeBackendOnExit:
          prefs.getBool('llm_studio.close_backend_on_exit') ??
          defaults.closeBackendOnExit,
      backendMode:
          prefs.getString('llm_studio.backend_mode') ?? defaults.backendMode,
      localPythonPath:
          prefs.getString('llm_studio.local_python_path') ??
          defaults.localPythonPath,
      localBackendRoot:
          prefs.getString('llm_studio.local_backend_root') ??
          defaults.localBackendRoot,
    );
  }
  ...
  Future<void> save(AppSettings settings) async {
    ...
    await prefs.setBool(
      'llm_studio.chat_streaming_enabled',
      settings.chatStreamingEnabled,
    );
    await prefs.setBool(
      'llm_studio.auto_start_backend',
      settings.autoStartBackend,
    );
    await prefs.setBool(
      'llm_studio.close_backend_on_exit',
      settings.closeBackendOnExit,
    );
    await prefs.setString('llm_studio.backend_mode', settings.backendMode);
    await prefs.setString(
      'llm_studio.local_python_path',
      settings.localPythonPath,
    );
    await prefs.setString(
      'llm_studio.local_backend_root',
      settings.localBackendRoot,
    );
  }
}
