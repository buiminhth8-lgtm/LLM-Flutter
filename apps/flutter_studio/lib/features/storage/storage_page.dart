  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        AppSectionHeader(
          title: 'Storage',
          subtitle: '清理前必须先 Preview；正式模型、外部模型、RAG 原始文档和全局 Hugging Face cache 默认受保护。',
          actions: [
            ...
            const SizedBox(width: 8),
            FilledButton.icon(onPressed: items.isEmpty ? null : onCleanup, icon: const Icon(Icons.cleaning_services), label: const Text('Execute cleanup')),
          ],
        ),
            ...
            Expanded(
              child: Card(
                child: categories.isEmpty
                    ? const AppEmptyState(title: '暂无存储统计', message: '点击刷新获取磁盘分类占用。')
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return ListTile(
                            ...
                            title: Row(
                              children: [
                                Expanded(child: Text('${map['name'] ?? 'category'}')),
                                AppStatusBadge(
                                  label: cleanable ? 'Cleanable' : 'Protected',
                                  tone: cleanable ? AppStatusTone.info : AppStatusTone.neutral,
                                ),
                              ],
                            ),
                            subtitle: Text('${map['size_bytes'] ?? 0} bytes'),
                          );
                        },
                      ),
              ),
            ...
            Expanded(
              child: Card(
                child: items.isEmpty
                    ? const AppEmptyState(
                        title: '未生成清理预览',
                        message: '先执行 Preview cleanup，确认待清理目录后才能执行 cleanup。',
