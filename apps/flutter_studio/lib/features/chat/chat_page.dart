    ...
    return Padding(
      ...
      child: Column(
        children: [
          AppSectionHeader(
            title: '聊天',
            subtitle: loaded ? '当前模型：$modelId' : '请先在模型页面加载模型。',
            actions: [
              AppStatusBadge(
              ...
              AppStatusBadge(label: adapter.isEmpty ? '适配器：无' : '适配器：$adapter'),
              const SizedBox(width: 12),
              const Text('流式输出'),
              Switch(
                value: controller.streamingEnabled,
                onChanged: controller.isGenerating ? null : onStreamingChanged,
              ),
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return _ChatBubble(
                          turn: turn,
                          generating:
                              index == controller.turns.length - 1 &&
          ...
          Row(
            children: [
              ...
              IconButton.filledTonal(
                onPressed:
                    controller.turns.any((turn) => turn.role == 'user') &&
                        !controller.isGenerating
                    ? onRegenerate
                    : null,
...
class _ChatBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final content = turn.content.isEmpty && generating
        ? '生成中...'
        : turn.content;
    return Align(
