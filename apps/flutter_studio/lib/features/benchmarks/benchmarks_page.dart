    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        AppSectionHeader(
          ...
          actions: [
            const AppStatusBadge(label: 'Experimental', tone: AppStatusTone.warning),
        ...
        FilledButton.icon(
          ...
          label: const Text('启动当前模型 Benchmark'),
        ),
        if (!loaded) ...[
          const SizedBox(height: 8),
          Text('请先加载模型。', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
        ],
        const SizedBox(height: 12),
        Expanded(
          child: benchmarks.isEmpty
              ? const AppEmptyState(
                  title: '没有 Benchmark 报告',
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                        [
                          ...
                          'Peak memory: ${map['peak_cuda_reserved'] ?? map['peak_vram'] ?? 'n/a'}',
                          if (map['error'] != null) 'Error: ${map['error']}',
                        ].join('  '),
                      ),
                    );
                  },
