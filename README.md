...
- Downloads：创建 ModelScope / 魔塔社区下载任务、查看真实进度、速度、ETA、取消请求、重试、删除记录，并在成功后自动扫描注册模型。
- RAG：最小查询入口。
...
ModelScope cache 默认不由 Storage Cleanup 删除。
...
| `model_download_resume` | partial | partial | retry 复用 ModelScope SDK cache，不声明严格暂停/续传 |
| `model_download_auto_register` | available | yes | 下载成功后自动触发扫描并把 `model_id` 写回任务 |
| `model_download_huggingface` | not_implemented | no | Hugging Face 远程下载 provider 已移除；本地 Transformers/HF 格式模型仍可扫描和加载 |
| `model_download_modelscope` | partial | yes | ModelScope / 魔塔 provider 可用；需要 `modelscope-hub`，部分进度字段可能未知 |
| `model_download_provider_selection` | not_implemented | no | Flutter 下载页面不再提供下载源选择，远程下载固定使用 ModelScope |
