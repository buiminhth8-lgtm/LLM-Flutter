...
pyinstaller --name llm-studio \
    ...
    --hidden-import=llm_studio.web_ui \
    --collect-all gradio \
...
# 运行 Web UI
docker run -p 7860:7860 -v $(pwd)/models:/app/models llm-studio

# 运行 API 服务
docker run -p 8000:8000 -v $(pwd)/models:/app/models llm-studio llm-studio serve

# GPU 支持（需要 nvidia-docker）
docker run --gpus all -p 7860:7860 -v $(pwd)/models:/app/models llm-studio
```
