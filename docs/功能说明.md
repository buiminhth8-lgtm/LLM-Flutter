...
工具同时提供可视化 **Web 界面**（Gradio）和完整的 **命令行 CLI** 两种使用方式。
...
| 实时训练进度 | Web UI 和 CLI 均提供 step/loss/lr 实时反馈 |
...
```bash
llm-studio ui                  # 默认端口 7860
llm-studio ui --port 8080      # 自定义端口
llm-studio ui --share          # 生成公网分享链接
```
