...
_CAPABILITIES: tuple[CapabilityInfo, ...] = (
    ...
    CapabilityInfo("model_download_huggingface", CapabilityStatus.AVAILABLE, "Hugging Face download provider remains supported.", True),
    CapabilityInfo("model_download_modelscope", CapabilityStatus.PARTIAL, "ModelScope provider is supported when modelscope-hub is installed; byte totals may be unavailable.", True),
    CapabilityInfo("model_download_provider_selection", CapabilityStatus.AVAILABLE, "Flutter can choose Hugging Face or ModelScope for model downloads.", True),
    CapabilityInfo("model_download_progress", CapabilityStatus.PARTIAL, "Progress uses Hugging Face file metadata when available; percent is null when total bytes are unknown.", True),
    ...
    CapabilityInfo("model_download_retry", CapabilityStatus.AVAILABLE, "Failed, cancelled, and interrupted downloads can be retried with Hugging Face cache reuse.", True),
    CapabilityInfo("model_download_resume", CapabilityStatus.PARTIAL, "Retry reuses the Hugging Face cache, but strict pause/resume is not claimed.", True),
